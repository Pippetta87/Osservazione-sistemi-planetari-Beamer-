\section{Photometry}\linkdest{emflux}
...
\section{Timing}\linkdest{timing}
...
\section{Microlensing gravitazionale.}\linkdest{microl}
...
\section{Tecniche spettroscopiche (Misura velocit\'a radiale).}\linkdest{RV}
...
\end{column} \begin{column}{0.7\textwidth}
\begin{figure}[!ht]\includegraphics[trim={0cm 0cm 0 0},clip, keepaspectratio,width=0.9\textwidth]{orbitalphase}\label{fig:orbitalphase}\end{figure}
\end{column}\end{columns}
\begin{align*}
...
\begin{wordonframe}{Ref detectability of RV survey}
Cumming 04: ''Detectability of extrasolar planets in radial velocity surveys''
\end{wordonframe}

\begin{frame}{Effetti di selezione}
\begin{columns}[c]\begin{column}{0.3\textwidth}
\begin{figure}[!ht]\includegraphics[trim={0cm 0cm 0 0},clip, keepaspectratio,width=0.9\textwidth]{sigma-jitter}\label{fig:sigma-jitter}\end{figure}
\begin{figure}[!ht]\includegraphics[trim={0cm 0cm 0 0},clip, keepaspectratio,width=0.99\textwidth]{detection-th}\label{fig:detection-th}\end{figure}
\end{column}
...
\begin{frame}{Detectability of RV survey (C04)}

\end{frame}

\section{Tecniche fotometriche: transiti.}\linkdest{T}

\begin{frame}{Observables of transit light curves}
\begin{columns}[c]\begin{column}{0.4\textwidth}
\begin{figure}[!ht]\includegraphics[trim={0cm 1cm 0 0},clip, keepaspectratio,height=0.41\textheight]{Tflux}\label{fig:Tflux}\end{figure}
\begin{figure}[!ht]\includegraphics[trim={0cm 0cm 0 0},clip, keepaspectratio,height=0.41\textheight]{Tflux-b-lambda}\label{fig:Tflux-b-lambda}\end{figure}
\end{column}
...
\begin{figure}[!ht]\includegraphics[trim={0cm 0cm 0 0},clip, keepaspectratio,width=0.99\textwidth]{Tprob}\label{fig:Tprob}\end{figure}
\end{column}
\begin{column}{0.7\textwidth}
...
\begin{frame}{Physical quantities}
\begin{columns}[T]
...
SNR: $s\approx\sqrt{N_t}\frac{\delta}{\sigma}$, con numero di osservazioni $N_t\approx N_{obs}(R_*/(\pi a))$: threshold for given detection probability and false alarm probability. Photon noise $\sigma\propto1/\sqrt{N_{\gamma}}\propto d$ quindi volume osservato $d^3\propto N_t\expy{3/2}\delta^3\propto R_p^6/P$, multiplied by transit probability  $\frac{R_*}{a}$ with constant star density, da number of detections $\propto R_p^6/P\expy{5/3}$.
\end{frame} 
