...
SNR: $s\approx\sqrt{N_t}\frac{\delta}{\sigma}$, con numero di osservazioni $N_t\approx N_{obs}(R_*/(\pi a))$: threshold for given detection probability and false alarm probability. Photon noise $\sigma\propto1/\sqrt{N_{\gamma}}\propto d$ quindi volume osservato $d^3\proptoN_t\expy{3/2}\delta^3\propto R_p^6/P$, multiplied by transit probability  $\frac{R_*}{a}$ with constant star density, da number of detections $\propto R_p^6/P\expy{5/3}$.
\end{frame} 
